<aside class="xl:w-1/4 lg:w-2/5 lg:flex flex-col hidden px-3">
    @if (auth()->check() && auth()->user()->isAdmin)
        ...
        <div class="flex flex-col max-h-screen overflow-y-scroll">
            @foreach ($classroomStats as $c)
                <div class="stat">
                    <legend class="title">کلاس {{ $c['id'] }}</legend>
                    <div>
                        <img src="/static/icons/users.png" class="unsature" alt="">
                        <strong>تعداد دانش‌آموزان</strong>
                        <p>{{ $c['count'] }}</p>
                    </div>
                    <div>
                        <img src="/static/icons/book.png" class="unsature" alt="">
                        <strong> پایه</strong>
                        <p>{{ $c['level'] }}</p>
                    </div>
                    <div>
                        <img src="/static/icons/chartGray.svg" class="convertToGray" alt="">
                        <strong> معدل کلاس </strong>
                        <p>{{ (floatval($c['avg']) == 0)? "بدون نمره":floatval($c['avg']) }}</p>
                    </div>
                    <div>
                        <img src="/static/icons/listGray.svg" class="convertToGray" alt="">
